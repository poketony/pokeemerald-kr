...
static void PrintOnPokeblockWindow(u8 windowId, u8 fontId, const u8 *string, s32 x)
{
    AddTextPrinterParameterized4(windowId, fontId, x, 1, 0, 0, sTextColorInPokeblockMenu, 0, string);
}
...
static void PutPokeblockInfoText(void)
{
    ...
    PrintOnPokeblockWindow(0, 7, itemName, GetStringCenterAlignXOffset(7, itemName, 0x48));

    PrintOnPokeblockWindow(2, 1, gText_Spicy, 0);
    PrintOnPokeblockWindow(3, 1, gText_Dry, 0);
    PrintOnPokeblockWindow(4, 1, gText_Sweet, 0);
    PrintOnPokeblockWindow(5, 1, gText_Bitter, 0);
    PrintOnPokeblockWindow(6, 1, gText_Sour, 0);
...
static void sub_8135FCC(s32 pkblId)
{
    ...
    if (pkblId != LIST_CANCEL)
    {
        ...
        PrintOnPokeblockWindow(7, 7, gStringVar1, 4);
    }
    else
    {
        rectTilemapSrc[0] = 0xF;
