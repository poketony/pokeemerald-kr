...
static void SetCursorPos(s16 x, s16 y)
{
    ...
    if (x < sPageColumnCounts[CurrentPageToKeyboardId()])
        cursorSprite->pos1.x = sPageColumnXPos[x + CurrentPageToKeyboardId() * KBCOL_COUNT] + 27;
    else
...
static void BufferCharacter(u8 ch)
{
    ...
    if (index >= 2)
    {
        prevChar = sNamingScreen->textBuffer[index - 2] << 8 |
...
static void PrintKeyboardKeys(u8 window, u8 page)
{
    ...
    {
        AddTextPrinterParameterized3(window, 7, 0, i * 16 + 1, sKeyboardTextColors[page], 0, sNamingScreenKeyboardText[page][i]);
    }
